...
class Firewall (EventMixin):
	...
	def _handle_ConnectionUp (self, event):
		''' Dont use hand_connection because the event doesn't
		 allow packet inspection ... '''
	...
	#this is where the port security happens. I added a port_table to the firewall up top
	def _handle_PacketIn(self, event):
		packet = event.parsed
		#arp packets will not have the same format so get out
		...
		'''
		There are 3 options for MAC/IP address.
		1. first time seeing it. map MAC/IP
		2. already seen MAC but the MAC/IP match
		3. Already seen MAC but IP is different. Spoofed
		'''
		#1.
		if src_mac not in self.port_table:
			self.port_table[src_mac] = src_ip
			print("Learning MAC %s and mapped to IP %s" % (src_mac, src_ip))
			return
		#2.
		if self.port_table[src_mac] == src_ip:
			print("Good valid traffic already seen MAC/IP")
			return
		#3.
		correct_ip = self.port_table[src_mac]
		print("DOS Attack from MAC: %s spoofedIP: %s should be: %s" % (src_mac, src_ip, correct_ip))
		'''
		create a new flow mod rule.  set the highest priority which is 65535, create a match
		object, define the source MAC we want to block with math.dl_src and send to the
		switch using event.connection.send(flow_mod)
		'''
		flow_mod = of.ofp_flow_mod()
		flow_mod.priority = 65535 #highest possible priority
		flow_mod.match = of.ofp_match()
		flow_mod.match.dl_src = match.dl_src
